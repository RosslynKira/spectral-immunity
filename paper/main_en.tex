...
% Final Version v10c - Compact (4-page hard limit)
% ============================================================
...
\usepackage[margin=1.25cm,columnsep=0.42cm,top=1.45cm,bottom=1.0cm]{geometry}
\usepackage{setspace}
\usepackage{fancyhdr}
\usepackage{float}
\usepackage{enumitem}
...
\titlespacing{\section}{0pt}{0.4pt}{0.1pt}
\titleformat{\subsection}[runin]{\small\bfseries}{}{0em}{}
\titlespacing{\subsection}{0pt}{0.8pt}{0.8pt}
...
{\scriptsize $^{1}$ Nankai High School, Tianjin, China \quad \texttt{mdrosslyning@gmail.com} \quad \texttt{daoming\_kan@163.com}\quad Supervisor: Wei Teng \ $^{1}$ }
    \vspace{0.3pt}
...
\end{equation}
where $C(\theta) = \frac{1}{|a_{\text{ref}}|} \sqrt{\frac{\int_{\Beff} \hsysf \, df}{|\Beff|}}$. \textit{Proof sketch:} By Parseval/Plancherel, $\operatorname{Var}[\hat{a}] = \int_0^{f_s/2} \hsysf \snf \,df$. Split the integral into $\Beff$ and its complement. On $\Beff$, write $\snf = S_0(1+\eta(f))$ with $|\eta|\le\delta$; then $\int_{\Beff}\hsysf\snf df = S_0\int_{\Beff}\hsysf df\,(1+O(\delta))$. The complement contributes $O(\epsilon)$ by (i). Since $\pxrmse^2 = \int \snf df = S_0|\Beff|(1+O(\delta+\epsilon))$, dividing yields $\operatorname{Var}[\hat{a}] = \pxrmse^2 \cdot \frac{1}{|\Beff|}\int_{\Beff}\hsysf df \,(1+O(\delta+\epsilon))$, giving the stated $C(\theta)$. $\square$
\end{theorem}
...
\textbf{Statistical note:} Bootstrap validation ($B=1000$): for $N=1000$, 95\% CI $\approx\pm10\%$. Welch leakage underestimates NPRF for concentrated spectra (zero-predictor: measured 5.731 vs.\ theoretical $\sim10^7$). For $N<200$: screening; $N<100$: concept only. \textbf{Spectral Vaccine} = \{NPRF diagnosis + Spectral-Immunity filter\}. Complexity: $O(N\log N)$, $<1$ ms for $N=10^4$.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.78\columnwidth]{1.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.78\columnwidth]{2.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.78\columnwidth]{3.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.78\columnwidth]{4.png}
...
\begin{center}
    ...
    \begin{tabular}{lcc}
        ...
        Workflow demo value & 75.3\% & SG instantaneous $a$ \\
...
\subsection{Experiment 7: Real Neural-Network NPRF Audit (Proof of Concept)}

\textbf{Setup:} We train a 1D CNN denoiser (4 conv layers, 32/32/16 channels, ReLU, Adam, 100 epochs) on \textbf{white-noise}-contaminated PGTS signals (10 Hz + 60 Hz, $\sigma=0.001$, $n=19900$ windows). We then apply it \textit{without retraining} to test signals contaminated by red ($\alpha=-2$), white ($\alpha=0$), and blue ($\alpha=+2$) noise ($\sigma=0.002$). For each condition, we extract window-center residuals and compute pixel RMSE, NPRF (using the SG operator frequency response), and physical error (estimated as $\|\text{SG}(\mathbf{r})\|/|a_{\text{ref}}|$, equivalent to Eq.~\eqref{eq:metrics} under PGTS constant acceleration; boundary frames excluded).

\begin{center}
    \scriptsize\setlength{\tabcolsep}{3.0pt}
    \begin{tabular}{lccc}
        \toprule
        \textbf{Test noise ($\alpha$)} & \textbf{Pixel RMSE} & \textbf{NPRF} & \textbf{Physical error (\%)} \\
        \midrule
        Red ($-$2) & 0.00198 & 0.213 & 118.9 \\
        White (0) & 0.00143 & $\mathbf{1.960}$ & $\mathbf{299.1}$ \\
        Blue ($+$2) & 0.00080 & 1.374 & 133.0 \\
        \bottomrule
    \end{tabular}
    \\[0.1pt]
    {\scriptsize \textbf{Table 9:} A real 1D CNN denoiser trained only on white noise exhibits test-noise-dependent residual spectra. Pixel-RMSE ranking (red $>$ white $>$ blue) is \textit{inconsistent} with physical-error ranking (white $>$ blue $>$ red): red noise has the highest pixel RMSE but the lowest physical error, while white noise has intermediate pixel RMSE but the highest physical error. NPRF correctly flags white- and blue-noise conditions as higher-risk than red.}
\end{center}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.78\columnwidth]{5.png}
    \caption{Real neural-network NPRF audit. (a) CNN residual PSDs under three test noises; resonant band shaded. (b) NPRF values. (c) Physical error. The CNN is trained only on white noise; the spectral blind-spot phenomenon manifests in a real neural network.}
    \label{fig:fig5}
\end{figure}

\textbf{Key finding:} A CNN denoiser trained only on white noise exhibits residual spectra strongly dependent on test-noise type. NPRF correctly ranks the physical risk of these residuals, demonstrating that the spectral blind-spot phenomenon is not an artifact of synthetic noise construction but manifests in a real neural network.

\textbf{Note on multi-seed variance.} Preliminary multi-seed variance analysis (not shown) reveals that NPRF and physical error are highly sensitive to training randomness (NPRF coefficient of variation 30--65\%), while pixel RMSE remains relatively stable (CV 11--35\%). This phenomenon itself further supports the core thesis: standard MSE training does not confer spectral stability, and pixel-level metrics mask this instability. Robust multi-seed validation is left for future work.
...
\textbf{Linear-operator assumption} may break for strongly non-linear operators. \textbf{Calibration dependency:} black-box pipelines require numerical perturbation. \textbf{Statistical power:} Experiment 3 marginal ($p=0.056$, power$\approx0.35$); ablation sufficient ($p=0.000042$). \textbf{Signal-noise overlap:} notch filters attenuate genuine 60 Hz signal. \textbf{Welch leakage:} screening only for $N<200$. \textbf{PGTS-to-real-world gap:} quantitative results inside PGTS only. \textbf{Single-architecture validation:} Exp.~7 validates only one CNN architecture (1D, 4-layer conv); residual spectral characteristics may differ across architectures (Transformer, UNet, etc.). NPRF complements UQ: UQ asks ``how uncertain?'' NPRF asks ``how physically dangerous?''
...
Core contributions: (1) spectral sufficiency theorem (Thm.\ \ref{thm:sufficiency}); (2) NPRF requiring no physical ground truth (Eq.\ \ref{eq:nprf_def}); (3) dual RMSE+NPRF paradigm; (4) early-warning (8.26$\times$ disparity); (5) post-hoc remedy (NPRF $-49.8\%$, $p=0.002$); (6) counter-intuitive Butterworth finding (+380.0\% NPRF); (7) dual MSE blind-spots with ablation ($p=0.000042$, $d=1.558$); (8) real-world proof-of-concept; (9) real neural-network proof-of-concept (NPRF ranking consistent with physical-error ranking, pixel-RMSE ranking inconsistent).

\begingroup
\scriptsize
\textbf{Boundary statement:} Proves: (1) pixel-RMSE structural failure under operator resonance; (2) NPRF feasibility; (3) MSE spectral blind-spots; (4) post-hoc reshaping efficacy in PGTS; (5) NPRF applicability on a real 1D CNN denoiser. Does \textbf{not} prove: (a) NPRF validity for arbitrary non-linear operators; (b) filter generalizability beyond PGTS; (c) training-noise-type significance ($p=0.056$, power$\approx0.35$); (d) NPRF generalizability across neural architectures.
\endgroup
...
\vspace{0.2pt}
{\scriptsize\noindent\textbf{Acknowledgements:} We thank Supervisor Wei Teng, head teachers Ms. Minglu Zhang and Mr. Zheng Niu, and grade director Ms. Qian Ren for their support. 
\vspace{0.5pt}
{\scriptsize\noindent\textbf{Author Contributions:} Rosslyn Yang and Ethan Lin contributed equally to this work. R.Y. proposed the research question, developed the theoretical framework, and designed the experiments. E.L. performed the frequency-domain analysis, implemented all code, and conducted the numerical experiments. Both authors analyzed the results and wrote the manuscript.}
Code: \texttt{https://github.com/RosslynKira/spectral-immunity}}
...
\bibitem{ronen2019}
R.~Ronen, R.~Basri, D.~Jacobs, Y.~Kasten, and S.~Kritchman.
