...
两位作者贡献均等。（1）\textbf{诊断}（\S 2.1）：证明代理指标有效当且仅当误差PSD在算子有效通带内平坦。（2）\textbf{工具}（\S 2.2）：提出NPRF，无需物理真值。（3）\textbf{早期预警}（\S 3.3）：演示NPRF标记像素RMSE无法检测的8.26倍物理误差差异。（4）\textbf{补救}（\S 3.5）：验证后处理频谱重塑。（5）\textbf{真实模型验证}（\S 3.7）：在真实训练的 1D CNN 去噪器上验证 NPRF 的适用性。
...
\begin{center}
    ...
    \end{tabular}
    \\[0.5pt]
    {\scriptsize \textbf{表4：}Kruskal-Wallis：$H=5.775$，$p=0.056$（未校正）。Bonferroni校正后（$\alpha=0.0167$）\textbf{均不显著}。事后功效$\approx0.35$；\textbf{边际方向性证据}；需$n\geq60$/组。消融（仅10 Hz vs.\ 10+60 Hz）：NPRF $1.167\to2.479$，2.12倍（$p=0.000042$，Cohen's $d=1.558$）。信号型盲区是主导因果因素。}
\end{center}
...
\subsection{实验7：真实神经网络 NPRF 审计（概念验证）}

\textbf{设置：}我们在\textbf{白噪声}污染的 PGTS 信号（10 Hz + 60 Hz，$\sigma=0.001$，$n=19900$ 窗口）上训练 1D CNN 去噪器（4 层卷积，32/32/16 通道，ReLU，Adam，100 epochs）。然后\textit{不重新训练}，直接应用于红（$\alpha=-2$）、白（$\alpha=0$）、蓝（$\alpha=+2$）噪声污染的测试信号（$\sigma=0.002$）。对每种条件，我们提取窗口中心残差，计算像素 RMSE、NPRF（使用 SG 算子频响）和物理误差（按式~\eqref{eq:metrics} 定义，其中 $a_{\text{ref}}$ 由 PGTS 匀加速轨迹给出；边界帧因 SG 滤波器边缘效应被排除）。

\begin{center}
    \scriptsize\setlength{\tabcolsep}{3.5pt}
    \begin{tabular}{lccc}
        \toprule
        \textbf{测试噪声（$\alpha$）} & \textbf{像素 RMSE} & \textbf{NPRF} & \textbf{物理误差（\%）} \\
        \midrule
        红（$-$2） & 0.00198 & 0.213 & 118.9 \\
        白（0） & 0.00143 & $\mathbf{1.960}$ & $\mathbf{299.1}$ \\
        蓝（$+$2） & 0.00080 & 1.374 & 133.0 \\
        \bottomrule
    \end{tabular}
    \\[0.5pt]
    {\scriptsize \textbf{表 9：}仅用白噪声训练的真实 1D CNN 去噪器，其残差频谱依赖于测试噪声类型。像素 RMSE 排序（红 $>$ 白 $>$ 蓝）与物理误差排序（白 $>$ 蓝 $>$ 红）\textit{不一致}：红噪声像素 RMSE 最高但物理误差最低，白噪声像素 RMSE 居中但物理误差最高。NPRF 正确地将白噪声和蓝噪声条件标记为比红噪声更高风险。}
\end{center}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\columnwidth]{5.png}
    \caption{真实神经网络 NPRF 审计。(a) 三种测试噪声下的 CNN 残差 PSD；共振带阴影标注。(b) NPRF 值。(c) 物理误差。CNN 仅用白噪声训练；频谱盲区现象在真实神经网络中显现。}
    \label{fig:fig5}
\end{figure}

\textbf{核心发现：}仅用白噪声训练的 CNN 去噪器，其残差频谱强烈依赖于测试噪声类型。NPRF 正确排序这些残差的物理风险，证明频谱盲区现象不是合成噪声构造的产物，而是在真实神经网络中显现。

\textbf{关于多 seed 方差的说明。}初步的多 seed 方差分析（未展示）显示，NPRF 与物理误差对训练随机性高度敏感（NPRF 变异系数达 30--65\%），而像素 RMSE 保持相对稳定（变异系数 11--35\%）。这一现象本身即进一步支持本文核心论点：标准 MSE 训练不提供频谱稳定性，而像素级指标掩盖了这种不稳定性。稳健的多 seed 验证留待未来工作。
...
\textbf{线性算子假设}对强非线性算子可能失效。\textbf{标定依赖性：}黑盒流程需要数值扰动。\textbf{统计功效：}实验3边际性（$p=0.056$，功效$\approx0.35$）；消融充分（$p=0.000042$）。\textbf{信号-噪声重叠：}陷波滤波器衰减真实60 Hz信号。\textbf{Welch泄漏：}$N<200$仅筛查。\textbf{PGTS到真实世界的差距：}定量结果仅在PGTS内。\textbf{单一架构验证：}实验7 仅验证了一种 CNN 架构（1D 4 层卷积），不同架构（Transformer、UNet 等）的残差频谱特性可能不同。NPRF补充UQ：UQ问“有多不确定？”NPRF问“在物理上有多危险？”
...
核心贡献：（1）频谱充分性定理（定理\ref{thm:sufficiency}）；（2）无需物理真值的NPRF（式\ref{eq:nprf_def}）；（3）RMSE+NPRF双范式；（4）早期预警（8.26$\times$差异）；（5）后处理补救（NPRF $-49.8\%$，$p=0.002$）；（6）反直觉Butterworth发现（+380.0\% NPRF）；（7）MSE双重盲区与消融（$p=0.000042$，$d=1.558$）；（8）真实世界概念验证；（9）真实神经网络概念验证（NPRF 排序与物理误差排序一致，像素 RMSE 排序不一致）。

\begingroup
\scriptsize
\textbf{边界声明：}已证明：（1）算子共振下像素RMSE的结构性失效；（2）NPRF可行性；（3）MSE频谱盲区；（4）PGTS内后处理重塑有效性；（5）真实 1D CNN 去噪器上的 NPRF 适用性。\textbf{未}证明：（a）NPRF对任意非线性算子的有效性；（b）滤波器在PGTS之外的泛化性；（c）训练噪声类型显著性（$p=0.056$，功效$\approx0.35$）；（d）NPRF 对不同神经网络架构的泛化性。
...
\vspace{1pt}
{\scriptsize\noindent 代码：\texttt{https://github.com/RosslynKira/spectral-immunity}}
